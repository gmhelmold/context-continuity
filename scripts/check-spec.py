...
def validate_reference_ddl():
    with sqlite3.connect(':memory:') as db:
        ...
        ch=dict(chapter_id='c1',session_key='one',operation_id='manual',digest='h',coverage_json='[]',logical_json='[]',dependencies_json='[]',parents_json='[]',manifest_json='{}',proposal_json='{}',status='published',created_at='fixture')
        add('chapters',**ch)
        rejected(lambda:add('chapters',**{**ch,'chapter_id':'cross','session_key':'two'}),'Cross-scope chapter')
        add('retrievals',retrieval_id='r1',session_key='one',chapter_id='c1',work_json='{}',range_json='{}',text_digest='h',reason='lookup',declared_by='agent',created_at='fixture',consumed_by_job='j1')
