...
function apply(roots,replacements) {
  const result=[];let at=0;
  const sorted=replacements.map(r=>({...r,start:roots.findIndex(x=>x.id===r.ids[0])})).sort((a,b)=>a.start-b.start);
  for(const r of sorted) {
    if(r.start<at||r.start<0) throw error("E_STALE_VIEW");
    const covered=roots.slice(r.start,r.start+r.ids.length);
    if(hash(covered.map(x=>[x.id,x.digest]))!==r.sourceHash) throw error("E_STALE_VIEW");
    result.push(...roots.slice(at,r.start).flatMap(x=>x.parts),{role:"assistant",content:r.summary});
    at=r.start+r.ids.length;
  }
  return [...result,...roots.slice(at).flatMap(x=>x.parts)];
}
...
export default async function gatewayPlugin(ctx) {
  ...
  const emit=(kind,data={})=>appendFileSync(trace,JSON.stringify({kind,at:Date.now(),...data})+"\n");
  const captures=new Map(),pending=new Map(),sessions=new Map();let closing=false;
  const state=id=>{if(!sessions.has(id))sessions.set(id,{epoch:0,view:[],job:null,seen:new Set()});return sessions.get(id);};
  ...
  async function auxiliary(s,capture,body,roots,config,control) {
    ...
    const removable=roots.filter(r=>!r.protected);
    // Probe interval comes from completed earlier turns, not the current request.
    const selected=removable.slice(0,2);
    if(selected.length!==2||selected.some(x=>x.parts.some(m=>m.tool_calls?.length))) throw error("PROBE_INTERVAL");
    const job={id,controller,terminal:false,epoch:s.epoch,config,ids:selected.map(x=>x.id),sourceHash:hash(selected.map(x=>[x.id,x.digest])),attempts:0,ready:null};
    s.job=job;
    const mission=`CC_AUXILIARY::${control.scenario}::${control.run}. Return JSON with a summary of the first completed turn. Do not call tools.`;
    ...
    finally {clearTimeout(timer);emit("aux-local-stopped",{session:capture.session,job:id});}
  }
      ...
      if(capture.kind==="primary") {
        ...
        if(s.job?.ready&&!s.job.terminal){
          ...
          if(job.epoch!==s.epoch||job.config!==config){job.terminal=true;emit("proposal-stale",{session:capture.session,job:job.id});}
            ...
            const r={ids:job.ids,sourceHash:job.sourceHash,summary:job.ready};
            const next=[...s.view.filter(v=>!v.ids.some(id=>r.ids.includes(id))),r];
            apply(roots,next);s.view=next;job.terminal=true;
            emit("view-applied",{session:capture.session,job:job.id,ids:r.ids});
          }
        }
        selected={...body,messages:apply(roots,s.view)};
        emit("sealed",{session:capture.session,kind:capture.kind,rawHash:hash(body),effectiveHash:hash(selected),rootIds:roots.map(x=>x.id)});
        if(control.session===capture.session&&control.run&&!s.seen.has(control.run)&&!s.job?.ready&&!s.job?.controller?.signal.aborted) {
          s.seen.add(control.run);
          void auxiliary(s,capture,selected,roots,config,control).catch(e=>emit("aux-setup-failed",{code:e.code??e.name}));
        }
      } else emit("other-request",{session:capture.session,kind:capture.kind});
  ...
  return {
    ...
    "chat.headers":async(input,output)=>{
      ...
      emit("correlated",{session:input.sessionID,kind:capture.kind});
    },
    "chat.message":async input=>{cancel(input.sessionID,"user-input");},
    "experimental.session.compacting":async input=>{cancel(input.sessionID,"native-compaction");state(input.sessionID).view=[];state(input.sessionID).epoch++;emit("native-start",{session:input.sessionID});},
    event:async({event})=>{if(event.type==="session.compacted")emit("native-ended",{session:event.properties?.sessionID});},
