...
def main():
 ...
 for name,mutation in [('control',None),*MUTATIONS.items()]:
  with tempfile.TemporaryDirectory(prefix='cc-oracle-mutation-') as folder:
   ...
   path=copy/TARGET;text=path.read_text();assert text.count(STATEMENT)==1,'Mutation insertion point changed'
   if mutation:path.write_text(text.replace(STATEMENT,STATEMENT+'\n        '+mutation,1))
   run=Path(folder)/'run'
 ...
 print(f'PASS: control + {len(MUTATIONS)} incorrect implementations distinguished by stock-host oracle.')
 return 0
