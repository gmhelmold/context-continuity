...
test('WP-01/C: control plus five incorrect context implementations are distinguished', {timeout:60000},()=>{
  for(const [name,file,from,to,expected] of cases){
    ...
    try{
      ...
      for(const t of ['context-fixtures.mjs','manifest.test.mjs','proposal.test.mjs','frame.test.mjs']) cpSync(join(root,'tests/core',t),join(dir,'tests/core',t));
      ...
      const run=spawnSync(process.execPath,['--experimental-strip-types','--test','--test-reporter=tap','tests/core/manifest.test.mjs','tests/core/proposal.test.mjs','tests/core/frame.test.mjs'],{cwd:dir,env,encoding:'utf8',timeout:20000,maxBuffer:2*1024*1024});
      assert.equal(run.error,undefined,name+': runner error is not detection');assert.equal(run.signal,null);
      assert.match(run.stdout,/^# tests 33$/m,name+': all selected tests must execute');
