...
for(const format of ['messages','contents']){
  test(`T30.contract Frame ${format}: same core seals whole final JSON without dropping native fields`,()=>{
    ...
    assert.equal(assertSealedFrame(f.binding,v),v);assertFrameUnchanged(f.binding,v,f.body,f.profile);
    f.body.custom_metadata.nested[0]=77;reject(()=>assertFrameUnchanged(f.binding,v,f.body,f.profile));
  ...
  test(`T06.contract Frame ${format}: every nonhistorical field is bound even if no special key names it`,()=>{
    const f=frameFixture(format),v=seal(f);
    for(const patch of [{custom_metadata:{new:'value'}},{unknown_provider_setting:true}]){
      const newer=sealFrame(f.binding,f.capture,f.registry,{...f.body,...patch},f.layout,f.profile);
      assert.notEqual(newer.config_digest,v.config_digest);
      assert.notEqual(newer.input_digest,v.input_digest);
    }
    reject(()=>sealFrame(f.binding,f.capture,f.registry,f.body,{...f.layout,variant:'different'},f.profile));
  });
}
test('T02.contract Frame: capture/registry/frame must agree on the entire session binding',()=>{
  ...
  reject(()=>sealFrame(b.binding,a.capture,b.registry,b.body,b.layout,b.profile));
  reject(()=>sealFrame(a.binding,a.capture,b.registry,a.body,a.layout,a.profile));
  reject(()=>assertSealedFrame(b.binding,seal(a)));
  for(const wrong of [{...a.binding,incarnation:id(99)},{...a.binding,host_epoch:1}]) reject(()=>sealFrame(wrong,a.capture,a.registry,a.body,a.layout,a.profile));
});
test('T06.contract Frame: a partial/serialized capture cannot authorize a sealed request',()=>{
  ...
  reject(()=>sealFrame(f.binding,clone(f.capture),f.registry,f.body,f.layout,f.profile));
  reject(()=>assertSealedFrame(f.binding,clone(seal(f))));
  reject(()=>sealFrame(f.binding,{...f.capture,kind:'maintenance'},f.registry,f.body,f.layout,f.profile));
});
test('T06.contract Frame: changed final payload is rejected without mutating root registry',()=>{
  ...
  body.messages[1].content='lost/replaced input';reject(()=>sealFrame(f.binding,f.capture,f.registry,body,f.layout,f.profile));
  assert.deepEqual(f.registry.exportState(),before);
  body.messages[1]={...f.body.messages[1],provider_metadata:{signature:'new'}};
  reject(()=>sealFrame(f.binding,f.capture,f.registry,body,f.layout,f.profile));
});
test('T06.contract Frame: exact group partition rejects omission, duplication and arbitrary ordering',()=>{
  ...
  for(const mutate of [l=>l.groups.pop(),l=>l.groups.reverse(),l=>l.groups[1].native_identity='native-0',l=>l.groups[0].count=0,l=>l.groups[0].count=99,l=>l.prefix_length=0,l=>l.history_key='missing',l=>l.system_keys=['missing'],l=>l.tool_keys=['messages']]){
    const layout=clone(f.layout);mutate(layout);reject(()=>sealFrame(f.binding,f.capture,f.registry,f.body,layout,f.profile));
  }
...
test('T06.contract Frame: identity aliases and missing registry roots cannot be guessed by text',()=>{
  ...
  reject(()=>sealFrame(f.binding,f.capture,empty,f.body,f.layout,f.profile));
  ...
  reject(()=>sealFrame(f.binding,c,f.registry,f.body,f.layout,f.profile));
});
...
test('T37.contract Frame: unknown control fields and unsupported binary envelopes fail closed',()=>{
  ...
  reject(()=>sealFrame(f.binding,f.capture,f.registry,f.body,{...f.layout,permit:true},f.profile));
  for(const val of [NaN,Infinity,-1,Number.MAX_SAFE_INTEGER+1]) reject(()=>sealFrame(f.binding,f.capture,f.registry,f.body,{...f.layout,input_estimate:val},f.profile));
});
...
test('T06.contract Frame: a changed system prefix is configuration, never ordinary history',()=>{
  ...
  const newer=sealFrame(f.binding,f.capture,f.registry,body,f.layout,f.profile);
  assert.notEqual(newer.system_digest,old.system_digest);
  assert.notEqual(newer.config_digest,old.config_digest);
  reject(()=>assertFrameUnchanged(f.binding,old,body,f.profile));
});
