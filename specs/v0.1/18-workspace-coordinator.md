...
Exceção estreita F3: `recoverStaging` de SPEC-25 não aposenta owner nem executa reconciliação geral. Pode liberar somente UMA reserva `kind='staging'` pré-arquivo, identificada por binding original e reservation_id, de owner original active comprovadamente sem detentor. Durante toda transação até COMMIT, tanto owner original inspecionado quanto participante de recovery precisam conservar arquivo original, identidade `{dev,ino}`, registro/metadado active, process_instance exato e seus locks comprovadamente detidos. A transição exige CAS exato de owner/process/identidade, reserva e envelope; qualquer outro kind, `managed_files`, path/digest não null ou estado físico recusa sem alteração. Esta exceção não permite liberar outra reserva, limpar arquivo, adotar/retire owner ou inferir morte geral.
