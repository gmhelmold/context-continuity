...
Cada conexão recebe owner_id novo. `acquireLease` usa transação SQLite, verifica binding e proprietário vigente, e incrementa owner_fence sem overflow. Prazo: 30s. Reaquisição do mesmo proprietário ainda válido não renova prazo; renewal é explícito. Dois processos não podem tomar a mesma sessão simultaneamente. `renewLease`/`releaseLease` exigem owner_id da conexão, fence e deadline exatos, além de validade temporal. Deadline antigo após renewal também perde validade. Release não zera o fence; takeover após expiração gera fence maior.
