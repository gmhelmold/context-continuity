...
`staging-intents.test.mjs` cobre forma pública, limites, imutabilidade, binding/policy/tombstone, replay, owner, row/envelope, limites UTF-8 de envelope e scope de sessão, quota com retenção inline, fence `managed_files`, cancelamento, atomicidade e consulta scalar de histórico cross-kind sem transferir payload estrangeiro. `staging-intents-processes.test.mjs` cobre crash antes/depois de `COMMIT`, dois participantes na última capacidade e reinício sem adoção. `staging-intents-mutations.test.mjs` controla quota, owner, replay/policy, bound do scope, fence físico, histórico cross-kind e materialização estrangeira; cada mutante deve falhar por assertion nomeada após controle passar.
